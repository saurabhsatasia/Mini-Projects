...
class Predict:
    ...
    def prediction(self):
        ...
        st.write(test_image.class_in)
        predictions = model.predict(test_image)
        score = tf.nn.softmax(predictions[0])
        # ['daniel_radcliffe', 'hermione_granger', 'professor_albus_dumbledore', 'professor_severus_snape', 'ron_wesley']
        if predictions[0][0] == 0: prediction = 'daniel_radcliffe'

        elif predictions[0][1] == 1: prediction = 'hermione_granger'

        elif predictions[0][2] == 2: prediction = 'professor_albus_dumbledore'

        elif predictions[0][3] == 3: prediction = 'professor_severus_snape'

        else: prediction = 'ron_wesley'
